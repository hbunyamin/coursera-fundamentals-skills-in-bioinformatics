...
ds3 <- read.csv(here::here( "CODING_WEEK_2", "data",  "DATA_FSB_SET_3.csv") , row.names = 1)
summary(ds3)
...
## Analyze assumptions
Before we can employ a t-Test, we have to assess whether the fundamental assumptions hold.

### Asumption 1: _each group follows a normal distribution_.

A histogram provides a useful visualization. Let's look at LDL level histogram:
...
But visual assessment can be deceiving. A more precise way is a quantile-quantile plot:
...
The third assumption is independence of samples from each other. 
This assumption needs to be investigated in the context of the
experimental design. Let us assume, individuals are coming from different
families, and there is no bias towards selection (e.g. not a bias towards younger).
...
As a result, considering that we set a theshold for significance at 0.01,
we can reject the null hypothesis, and we will consider the two populations
to have a different mean.
